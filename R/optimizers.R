...
#'
#' @param fn Objective `function(par, ...)`.
#' @param par Starting vector.
#' @param lower,upper Bounds.
#' @param control `maxeval` (default 200), `r` (default 0.2), optional `seed`.
#' @param ... Passed to `fn`.
#' @return List with `par`, `value`, `evaluations`, `method`.
#' @keywords internal
...
#' Space-filling starts for multi-start BOBYQA
#' @noRd
...
#' Default evaluation budget used by the reporter (`n_total`)
#' @noRd
...
#' Population size for reporter `np` (1 for sequential methods)
#' @noRd

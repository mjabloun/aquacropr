...
test_that("generate_param_design accepts a list and a data.frame", {
...
test_that("generate_param_design works for 1 parameter and many", {
  ...
  many_params <- data.frame(name = paste0("x", seq_len(12)), min = 0, max = 1)
...
test_that("generate_param_design dice_lhs is in bounds and not extensible", {
...
test_that("run_param_design archives renamed files and read_param_design reloads them", {
  ...
  fixture <- test_path("fixtures", "wheatCROP.OUT")

  work <- withr_local_tempdir()
  keep <- file.path(work, "archive")
  out_rel <- file.path("OUTP", "wheatCROP.OUT")
  dir.create(file.path(work, "OUTP"), recursive = TRUE)
  file.copy(fixture, file.path(work, out_rel))

  design <- data.frame(CCx = c(0.90, 0.95), WP = c(14, 16))

  local_mocked_bindings(
    render_templates = function(...) invisible(character()),
    run_aquacrop = function(...) invisible(0L)
  )

  res <- run_param_design(
    design, parameters,
    output_files = c(crop = out_rel),
  ...
  expect_true(file.exists(file.path(keep, "run_0001_wheatCROP.OUT")))
  expect_true(file.exists(file.path(keep, "run_0002_wheatCROP.OUT")))
  ...
  expect_true("CC" %in% names(loaded$outputs))
  expect_false("CCx" %in% names(loaded$outputs))
...
test_that("run_param_design requires reading when files are not kept", {
  ...
  expect_error(
    run_param_design(
      data.frame(CCx = 0.9, WP = 15),
      parameters,
      output_files = c(crop = "OUTP/wheatCROP.OUT"),
...
test_that("run_param_design can read in-loop with a custom reader", {
  ...
  fixture <- test_path("fixtures", "wheatCROP.OUT")
  work <- withr_local_tempdir()
  dir.create(file.path(work, "OUTP"))
  file.copy(fixture, file.path(work, "OUTP", "wheatCROP.OUT"))

  design <- data.frame(CCx = 0.91, WP = 15.5)
  ...
  res <- run_param_design(
    design, parameters,
    output_files = c(crop = file.path("OUTP", "wheatCROP.OUT")),
